...
var queryChaincode = async function(peer, channelName, chaincodeName, args, fcn, username, org_name) {
	...
	try {
		...
		if (response_payloads) {
			logger.debug('Successfully retrieved response payload.');
			response.success = true;
			response.payload = JSON.parse(response_payloads[0].toString());
		} else {
			logger.error('response_payloads is null');
			response.success = false;
			response.message = 'response_payloads is null';
		}
	} catch(error) {
		logger.error('Failed to query due to error: ' + error.stack ? error.stack : error);
		response.success = false;
		response.message = 'Failed to query due to error: ' + error.toString();
	}
